...
class classifier(nn.Module):
    ...
    def __init__(self):
        ...
        # self.ltsm = nn.LSTMCell(400, 400)

        self.fc1 = nn.Linear(400,25) # should be 512*2?
...
net = torchvision.models.video.r2plus1d_18(pretrained=False, progress=True, num_classes=8) # , num_classes = 8
# num_ftrs = net.fc.in_features
# net.fc = nn.Linear(num_ftrs, 8)  # make the change
...
train_loader, test_loader = readdata_depth.MakeDataloaders(test_dir="../nvGesture/nvgesture_test_correct_cvpr2016.lst", train_dir="../nvGesture/nvgesture_train_correct_cvpr2016.lst")


def saveModel(acc): 
    ct = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S_")
    path = "./network_2P1_pretrain1000_" + ct + str(acc) + "_.pth"
